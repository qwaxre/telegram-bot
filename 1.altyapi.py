...
async def sms(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if len(update.message.text.split(" ")) == 2 and update.message.text.split(" ")[0] == "/sms":
        if len(update.message.text.split(" ")[1]) == 10:
            ...
            await update.message.reply_document("https://media.tenor.com/-ED2EtbefSIAAAAC/suleyman-cakir-kurtlar-vadisi.gif", f"*{adet} adet SMS Gönderiliyor \-\-\> {telno}*", parse_mode="MarkdownV2")
            sms = SendSms(telno, "")
            while sms.adet < adet:
...
def main() -> None:
    application = Application.builder(6345381785:AAFWNjhYIqfG7r53-J13lQTO5Dfa97tnZ5c).token(6345381785:AAFWNjhYIqfG7r53-J13lQTO5Dfa97tnZ5c).build(6345381785:AAFWNjhYIqfG7r53-J13lQTO5Dfa97tnZ5c)
)
